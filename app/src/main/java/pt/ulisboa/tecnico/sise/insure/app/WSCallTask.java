...
//template para adaptar a cada uma das nosssas actividades, call task só para fazer o login
...
public class WSCallTask extends AsyncTask<Void, String, Void> { //input, progress, retorno
    ...
    protected Void doInBackground(Void... params) {
        ...
        try {
            int claimId = 1;
            List<ClaimMessage> claimMessageList = WSHelper.listClaimMessages(sessionId, claimId); //mediador do acesso ao servidor
            if (claimMessageList != null) {
                String m = claimMessageList.size() > 0 ? "" : "empty array";
                for (ClaimMessage cm : claimMessageList ) {
    ...
    @Override
    protected void onProgressUpdate(String... values) { //exectutar codigo na interface do utilizador
        if(values.length > 0) {
    ...
    @Override
    protected void onPostExecute(Void result) { //Utilizado para
